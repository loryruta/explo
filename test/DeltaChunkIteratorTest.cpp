...
TEST_CASE("DeltaChunkIterator-checkLoadUnloadOnOffset")
{
	...
	WorldView::iterate_chunks(old_chunk_pos, render_distance, [&](glm::ivec3 const& chunk_pos)
	{
	...
	DeltaChunkIterator old_chunks_iterator(new_chunk_pos, old_chunk_pos, render_distance, [&](glm::ivec3 const& chunk_pos)
	{
		loaded_chunks.erase(chunk_pos);
	});
	old_chunks_iterator.iterate();

	DeltaChunkIterator new_chunks_iterator(old_chunk_pos, new_chunk_pos, render_distance, [&](glm::ivec3 const& chunk_pos)
	{
		loaded_chunks.emplace(chunk_pos);
	});
	new_chunks_iterator.iterate();

	WorldView::iterate_chunks(new_chunk_pos, render_distance, [&](glm::ivec3 const& chunk_pos)
	{
	...
	WorldView::iterate_chunks(old_chunk_pos, render_distance, [&](glm::ivec3 const& chunk_pos)
	{
		if (!WorldView::is_chunk_position_inside(new_chunk_pos, render_distance, chunk_pos))
			REQUIRE(!loaded_chunks.contains(chunk_pos));
	});
