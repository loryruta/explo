...
namespace explo
{
	...
	public:
		...
		using OctreeTraversalCallbackT = std::function<void(OctreeNode const& node, uint32_t level, uint64_t morton_code)>;

	private:
		glm::ivec3 m_octree_size; ///< The number of voxels along each octree axis.
		int m_octree_resolution;  ///< The number of levels of the octree.
	...
	public:
		...
		int get_octree_resolution() const { return m_octree_resolution; };
