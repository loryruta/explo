...
BakedWorldViewCircularGrid::BakedWorldViewCircularGrid(Renderer &renderer, glm::ivec3 const &render_distance) :
    m_renderer(renderer),
...
{
    assert(pos.x >= 0 && pos.x < m_side.x && pos.y >= 0 && pos.y < m_side.y && pos.z >= 0 && pos.z < m_side.z);
...
void BakedWorldViewCircularGrid::write_pixel(glm::ivec3 const &pos, Pixel const &pixel)
{
    ...
    // We split the pixel that we have to write into two pixels that lie
    // side-by-side, along the X axis, in the final image
...
BakedWorldView::BakedWorldView(Renderer &renderer, glm::ivec3 const &init_position, glm::ivec3 const &render_distance) :
    m_renderer(renderer),
...
bool BakedWorldView::is_chunk_position_inside(glm::ivec3 const &chunk_pos) const
{
    ...
    return rel_pos.x >= 0 && rel_pos.x < side.x && rel_pos.y >= 0 && rel_pos.y < side.y && rel_pos.z >= 0 && rel_pos.z < side.z;
}
...
size_t BakedWorldView::place_data(DeviceBuffer &buffer, VirtualAllocator &allocator, void *data, size_t data_size)
{
    ...
    // If a resize was done, make sure the buffer size matches the virtual
    // allocator size
    if (buffer.get_size() != allocator.get_size()) buffer.resize(allocator.get_size());
...
{
    // The user asked to upload a chunk that is outside the world view. This can happen frequently because the chunk
    // construction is asynchronous and if the player is travelling through the world fast, chunks could be built when they're
    // no longer inside the world view
    ...
    // The chunk doesn't have the surface! Instead of throwing, we silently
    // ignore the uploading
    if (!surface || surface->m_vertices.empty() || surface->m_indices.empty() || surface->m_instances.empty()) return;

    size_t vertex_offset =
        place_data(m_vertex_buffer, m_vertex_buffer_allocator, surface->m_vertices.data(), surface->m_vertices.size() * sizeof(SurfaceVertex));

    size_t index_offset =
        place_data(m_index_buffer, m_index_buffer_allocator, surface->m_indices.data(), surface->m_indices.size() * sizeof(SurfaceIndex));

    size_t instance_offset = place_data(
        m_instance_buffer, m_instance_buffer_allocator, surface->m_instances.data(), surface->m_instances.size() * sizeof(SurfaceInstance)
    );
...
{
    // If the chunk asked to be destroyed isn't covered by the world view anymore, we have lost references to it (that are stored in the circular
    // image): we can't free its geometry!
