...
void DebugUi::display_world_view_window()
{
    ...
    if (ImGui::Begin("World view"))
    {
        ...
        for (int x = 0; x < world_view_side.x; x++)
        {
            ...
            {
                glm::ivec3 chunk_pos(x - render_distance.x + player_chunk_pos.x, player_chunk_pos.y, z - render_distance.z + player_chunk_pos.z);
                ...
                if (world.is_chunk_loaded(chunk_pos))
                {
                    ...
                    // else if (chunk->has_volume()) rect_color = 0xFF00FFFF; // Yellow, with volume
                        ...
                        rect_color = 0xFF90FFCC;  // Green, just loaded
                }

                draw_list->AddRectFilled(
                    ImVec2(win_pos.x + x * rect_size.x + rect_pad.x * x, win_pos.y + z * rect_size.y + rect_pad.y * z),
                    ImVec2(win_pos.x + x * rect_size.x + rect_size.x + rect_pad.x * x, win_pos.y + z * rect_size.y + rect_size.y + rect_pad.y * z),
                    rect_color,
                    0.0f,
                    ImDrawFlags_None
                );
        ...
        draw_list->AddRectFilled(
            ImVec2(player_rect_pos.x - player_rect_size.x / 2, player_rect_pos.y - player_rect_size.y / 2),
            ImVec2(player_rect_pos.x + player_rect_size.x / 2, player_rect_pos.y + player_rect_size.y / 2),
            0xFF0000FF,
            5.0f,
            ImDrawFlags_None
