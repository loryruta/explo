...
void CullWorldView::record(VkCommandBuffer cmd_buf, vren::resource_container &res_container)
{
    ...
    m_pipeline.push_constants(
        ...
        &m_renderer.m_camera.m_position,
        sizeof(m_renderer.m_camera.m_position),
        offsetof(PushConstants, m_camera)
    );
    m_pipeline.push_constants(
        cmd_buf, VK_SHADER_STAGE_COMPUTE_BIT, &circular_grid.m_start, sizeof(circular_grid.m_start), offsetof(PushConstants, m_world_view_start)
    );
    m_pipeline.push_constants(
        cmd_buf,
        VK_SHADER_STAGE_COMPUTE_BIT,
        &circular_grid.m_render_distance,
        sizeof(circular_grid.m_render_distance),
        offsetof(PushConstants, m_render_distance)
    );
    ...
    m_pipeline.acquire_and_bind_descriptor_set(
        ...
        [&](VkDescriptorSet descriptor_set)
        {
            ...
            vren::vk_utils::write_buffer_descriptor(
                ...
                Renderer::k_chunk_draw_list_buffer_size,
                0
            );
    ...
    res_container.add_resources(circular_grid.m_gpu_image->get_image());
}
...
vren::render_graph_t CullWorldView::create_render_graph_node(vren::render_graph_allocator &allocator)
{
    ...
    node->set_callback(
        [this](uint32_t frame_idx, VkCommandBuffer cmd_buf, vren::resource_container &res_container)
        {
