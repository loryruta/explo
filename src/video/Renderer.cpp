...
Renderer::Renderer(GLFWwindow *window) :
    ...
    m_context(
        []()
        {
            vren::context_info context_info{
                .m_app_name = "explo", .m_app_version = VK_MAKE_VERSION(1, 0, 0), .m_layers = {}, .m_extensions = {}, .m_device_extensions = {}};
    ...
    m_presenter(
        m_context,
        m_surface,
        [this](vren::swapchain const &swapchain)
        {
    ...
    m_imgui_renderer(
        m_context,
        vren::imgui_windowing_backend_hooks{
            .m_init_callback =
                [this]()
            {
                ImGui_ImplGlfw_InitForVulkan(m_window, true);
            },
            .m_new_frame_callback =
                []()
            {
    ...
    m_chunk_draw_list(vren::vk_utils::create_device_only_buffer(
        m_context, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT, nullptr, k_chunk_draw_list_buffer_size
    )),
    m_chunk_draw_list_idx(vren::vk_utils::create_device_only_buffer(
        m_context, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT, nullptr, sizeof(uint32_t)
    )),

    m_cull_world_view(*this),
    m_draw_chunk_list(*this)
{
    vren::directional_light *directional_lights = m_light_array.m_directional_light_buffer.get_mapped_pointer<vren::directional_light>();
...
{
    return glm::uvec2(m_presenter.get_swapchain()->m_image_width, m_presenter.get_swapchain()->m_image_height);
}
...
void Renderer::on_swapchain_change(vren::swapchain const &swapchain)
{
    ...
    m_color_buffer = std::make_shared<vren::vk_utils::color_buffer_t>(vren::vk_utils::create_color_buffer(
        m_context,
        width,
        height,
    ...
    m_depth_buffer = std::make_shared<vren::vk_utils::depth_buffer_t>(vren::vk_utils::create_depth_buffer(
        m_context, width, height, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT
    ));
...
)
{
    ...
    render_graph.concat(m_cluster_and_shade(
        m_render_graph_allocator, screen, m_camera, *m_gbuffer, *m_depth_buffer, m_light_array, m_material_buffer, *m_color_buffer
    ));
    ...
    // Draw debug lines
    render_graph.concat(m_debug_renderer.render(
        m_render_graph_allocator,
        render_target,
        vren::camera_data{
    ...
    */
    ...
    render_graph.concat(vren::blit_color_buffer_to_swapchain_image(
        m_render_graph_allocator,
        *m_color_buffer,
...
void Renderer::upload_block_registry(BlockRegistry const &block_registry)
{
    ...
    if (block_registry.size() > 0)
    {
        ...
        for (BlockData const &block_data : block_registry.get_block_data()) image_data.push_back(block_data.m_color);
...
void Renderer::render()
{
    m_presenter.present(
        ...
        {
            on_frame(frame_idx, swapchain_image_idx, swapchain, command_buffer, resource_container);
        }
    );
}
