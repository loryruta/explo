...
namespace explo
{
	...
	public:
		...
		enum OpType : uint32_t { OP_WRITE_REGION };
	...
	public:
		explicit DeviceImage3d(Renderer& renderer,
			glm::ivec3 const& size,
			VkFormat format,
		...
		void write_region(
			glm::ivec3 const& region_position,
			glm::ivec3 const& region_size,
			void* data,
			size_t data_size
			);
	...
	private:
		void perform_write_region(
			VkCommandBuffer command_buffer,
			vren::resource_container& resource_container,
			WriteRegionOp const& write_region_op
			);
